...
class AudioGenerator:
    ...
    def generate_noise(self, duration: float, amplitude: float = 0.5) -> np.ndarray:
        """Generate Pink Noise (1/f) which is more realistic than white noise."""
        # Pink noise has equal power in bands that are proportionally wide.
        # Power density decreases with increasing frequency (1/f).

        num_samples = int(self.sample_rate * duration)

        # 1. Generate white noise in frequency domain
        # Use simple white noise generation as base
        white = np.random.randn(num_samples)

        # 2. FFT to frequency domain
        X_white = np.fft.rfft(white)

        # 3. Apply 1/f filter (1/sqrt(f) for amplitude)
        # Avoid division by zero at DC component (index 0)
        frequencies = np.arange(len(X_white))
        # Filter: 1/sqrt(f)
        # We start from 1 to avoid div/0. DC component is left alone or zeroed?
        # Let's just scale it.
        scaling = np.ones(len(X_white))
        ...
        # Ensure exact length (irfft can vary by 1 sample depending on odd/even input)
